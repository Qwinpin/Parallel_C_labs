...
void color_single(int **a, int *vert, int end){
	vert[end] = 1;
	for (int i = 0; i < end; i++){
		if (a[i][end] != 0 && vert[end] == vert[i]){
			vert[end] = vert[i] + 1;
		}
	}
}
...
void coloring(int **a, int *vert, int size, int th){
	int i, j;
	int step = size / th;
	...
	#pragma omp parallel for num_threads(th)
	for (i = 0; i < size; i++){
		pool[i] = (int*) malloc(size * sizeof(int));
	}
	}
...
void test(){
	...
	int tmp;
	srand(time(NULL));
	FILE *f = fopen("color_parallel_process.txt", "w");
	if (f == NULL)
	{
